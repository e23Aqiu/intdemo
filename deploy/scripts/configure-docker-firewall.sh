...
mode="${1:-${INTDEMO_INGRESS_MODE:-public}}"
admin_cidr=""
client_cidrs=""
http_mode="allow-http"
if [ "${mode}" = "public" ] || [ "${mode}" = "--public" ]; then
  mode="public"
  http_mode="${2:-allow-http}"
elif [ "${mode}" = "private" ] || [ "${mode}" = "--private" ]; then
  mode="private"
  admin_cidr="${2:-${ADMIN_PUBLIC_CIDR:-}}"
  client_cidrs="${3:-}"
  http_mode="${4:-deny-http}"
else
  # Backward-compatible form:
  # configure-docker-firewall.sh ADMIN_CIDR [CLIENT_CIDRS] [allow-http]
  admin_cidr="${mode}"
  client_cidrs="${2:-}"
...
if [ "${mode}" = "private" ] && [ -z "${admin_cidr}" ]; then
  echo "usage: sudo $0 public [allow-http|deny-http]" >&2
  echo "   or: sudo $0 private ADMIN_PUBLIC_IP/32 [CLIENT_CIDR[,CLIENT_CIDR...]] [allow-http]" >&2
  exit 2
fi
