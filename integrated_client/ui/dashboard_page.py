...
class DashboardMetricCard(QFrame):
    ...
    def __init__(self, title, symbol, tone="teal", parent=None):
        ...
        badge = QLabel(symbol)
        badge.setObjectName("DashboardMetricBadge")
        badge.setAlignment(Qt.AlignCenter)
        badge.setFixedSize(42, 42)
        badge.setStyleSheet(
            f"color:{accent};background:{badge_background};"
            "border:none;border-radius:11px;font-size:18px;font-weight:800;"
        )
...
class PhoneDistributionChart(AnimatedDonutChart):
    ...
    def paintEvent(self, event):
        ...
        painter.drawText(hole.adjusted(0, -12, 0, 0), Qt.AlignCenter, "总查询量")
        painter.setPen(QColor("#173a3d"))
        painter.setFont(QFont("Microsoft YaHei UI", 18, QFont.Bold))
        painter.drawText(hole.adjusted(0, 15, 0, 0), Qt.AlignCenter, f"{total:,}")
...
class StationShareChart(AnimatedDonutChart):
    ...
    def paintEvent(self, event):
        ...
        visible_rows = self._rows[:6]
        row_height = 27
        legend_height = len(visible_rows) * row_height
        start_y = available.center().y() - legend_height / 2
        painter.setFont(QFont("Microsoft YaHei UI", 8))
        for index, row in enumerate(visible_rows):
            ...
            painter.setPen(Qt.NoPen)
            painter.setBrush(color)
            painter.drawRoundedRect(QRectF(legend_x, y + 5, 10, 10), 2, 2)
            painter.setPen(QColor("#526e6c"))
            painter.drawText(
                QRectF(legend_x + 16, y, legend_width - 16, 20),
                Qt.AlignLeft | Qt.AlignVCenter,
                str(row.get("station", "")),
            )
            ...
            painter.setPen(color)
            painter.drawText(
                QRectF(legend_x + 16, y + 13, legend_width - 16, 16),
                Qt.AlignLeft | Qt.AlignVCenter,
...
class DashboardPage(QWidget):
    ...
    def __init__(self, database: Database, account: Account, parent=None):
        ...
        cards = (
            ...
            ("today", "今日查询量", "今", "blue"),
            ("phone", "有电话数量", "☎", "green"),
            ("no_phone", "无电话数量", "∅", "orange"),
            ("time", "总用时", "时", "purple"),
        )
        for column, (key, title_text, symbol, tone) in enumerate(cards):
            card = DashboardMetricCard(title_text, symbol, tone)
