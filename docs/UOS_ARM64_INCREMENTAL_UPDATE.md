...
当正式发布只能在 Windows 完成时，Windows 发布器用“导出已发布基线包”把真实
旧 DEB 与 `publish-receipt.json` 封装为 `uos-delta-base-<版本>.zip`；该传输包会
包含完整旧 DEB，所以大小大致相当于一个 DEB。UOS 发布器用“导入已发布基线包”
逐项校验后恢复本地产物库，拒绝覆盖同版本不同哈希基线。随后目标完整 DEB、
`.intdelta` 生成和 ARM64 引擎回放都在 UOS 真机进行，标准 UOS 结果 ZIP 再带回
Windows 导入发布。Windows 导入时会从结果元数据自动恢复 `uos_delta_from_version`。

差异算法本身只处理字节，并非理论上只能运行于 UOS；这里要求真机生成和回放，
是为了让正式候选同时覆盖真实 ARM64 补丁引擎、UOS 目标 DEB、内存/耗时、逐字节
重建和 50% 门槛，Windows 不生成或补签正式 UOS 增量产物。
...
1. 先升级更新清单服务和 Caddy 路由，加入 UOS 能力协商以及可选的按当前版本
   定向分发。该更改为加法扩展，未启用定向字段时旧客户端仍获得完整包。
...
- 定向清单只对精确列出的 v1.1.0/v1.1.1 当前版本返回更新，其他版本和无版本请求
  返回 HTTP 204；不带定向字段时行为保持原样。
- 发布器在旧 API/Caddy 上拒绝发布定向清单，避免旧服务器忽略字段后意外全量分发。
- UOS 已发布基线包导出、跨机器导入、哈希冲突拒绝和结果包增量来源自动识别正常。
...
- [x] 发布器支持 Windows 导出真实 UOS 已发布基线、UOS 导入并把验证结果带回
  Windows 发布。
