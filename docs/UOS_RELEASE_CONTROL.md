...
正式清单始终保留完整 DEB，v1.2.2 建立三层基线后可在 UOS ARM64 真机为一个精确
来源版本生成可选 `.intlayer`。来源无布局、引导文件变化或分层包过大时自动只保留
完整 DEB。
...
### 5.1 Windows 发布、UOS 真机制作分层更新包

当 Windows 是唯一能连接发布服务器的主控时，UOS 补丁按以下方式传递：

1. 已发布来源版本 `N` 后，在 Windows 发布器填写“UOS 分层来源”为 `N`，点击
   “导出已发布基线包”。得到
...
3. 在 UOS 上构建目标版本 `N+1`。发布器比较业务程序、稳定运行库和 Chromium
   三层并立即组装回放；结果必须与目标布局一致，且分层包必须小于完整 DEB 的
   50%。不满足时自动输出完整包回退结果，不会中断结果 ZIP 导出。
4. 把 UOS 标准结果 ZIP 带回 Windows，点击“导入统信构建包”。发布器自动识别
   `N`，同步“UOS 分层来源”，并校验 DEB、分层包或完整包回退报告。
5. Windows 取得同提交的 Windows 结果后统一发布。不要单独复制或手工选择裸
   `.intlayer`，也不要在 Windows 上制作正式 UOS 分层包。

v1.2.2 是首个带三层布局和稳定启动器的完整基线。v1.2.1 及更早来源会自动回退
完整 DEB；首次分层候选为 `v1.2.2 -> 后续版本`。v1.1.0/v1.1.1 始终只能取得
完整 DEB，其账号和登录会话不参与更新数据迁移。
...
- UOS 分层来源为空，或真实发布基线、分层包/完整回退报告全部一致；
...
- 发布 UOS 分层包时，API 必须声明 `uos-layered-v1`；
