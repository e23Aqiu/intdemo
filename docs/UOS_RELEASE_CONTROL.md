...
### 5.1 Windows 发布、UOS 真机制作逐文件更新包

当 Windows 是唯一能连接发布服务器的主控时，UOS 补丁按以下方式传递：

1. 已发布来源版本 `N` 后，在 Windows 发布器填写“UOS 逐文件来源”为 `N`，点击
   “导出已发布基线包”。得到
   `dist/uos-delta-bases/N/uos-delta-base-N.zip` 和对应 `.sha256`。
2. 把基线包带到 UOS ARM64 真机，点击“导入已发布基线包”。它会恢复真正发布的
...
5. Windows 取得同提交的 Windows 结果后统一发布。不要单独复制或手工选择裸
...
- UOS 缺少真实增量基线：在保存来源版本发布收据的 Windows 主控导出“已发布
  基线包”，再到 UOS 导入；不要重新构建旧版本。
