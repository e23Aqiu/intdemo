...
正式发布需要系统提供 `ssh`、`sftp`，并允许 UOS 发布机访问发布服务器；远程构建
或暂停分发还会使用 `scp`。它们通常都由 `openssh-client` 提供。执行“运行全部
测试”或一键完整流程时，还要准备 `server/.venv`；只构建 DEB、导出/导入 Windows
任务时不需要 Docker。
...
配置只保存地址和文件路径（包括 Gitee 仓库链接）以及定向更新版本列表，不保存
更新说明、强更选择、密码、令牌或私钥内容。
...
正式清单始终保留完整 DEB。v1.2.4 建立逐文件基线后，可在 UOS ARM64 真机为一个
精确来源版本生成可选 `.intlayer`。来源无新布局、引导文件变化或补丁
过大时自动只保留完整 DEB。
...
### 5.1 双端基线传递与 UOS 真机制作逐文件更新包

当构建和正式发布分属不同主机时，使用对应平台的基线包传递；UOS 逐文件补丁仍按
以下方式在 ARM64 真机制作：

1. 已发布来源版本 `N` 后，在任一保存真实发布资料的主控填写“UOS 逐文件来源”为
   `N`，点击“导出 UOS 基线包”。得到
   `dist/uos-delta-bases/N/uos-delta-base-N.zip` 和对应 `.sha256`。
2. 把基线包带到 UOS ARM64 真机，在任一发布器点击“导入 UOS 基线包”。它会恢复真正发布的
   来源 DEB 和发布收据，不接受重建的旧包或同版本不同哈希文件。
3. 在 UOS 上构建目标版本 `N+1`。发布器根据文件布局只携带新增和变化文件，并把
   删除路径写入清单后立即组装回放；结果必须与完整目标布局一致，且补丁必须小于完整 DEB 的
   50%。不满足时自动输出完整包回退结果，不会中断结果 ZIP 导出。
4. 把 UOS 标准结果 ZIP 带回 Windows，点击“导入统信构建包”。发布器自动识别
   `N`，同步“UOS 逐文件来源”，并校验 DEB、逐文件包或完整包回退报告。
5. 任一主控取得同提交的 Windows 结果后统一发布。不要单独复制或手工选择裸
   `.intlayer`，也不要在 Windows 上制作正式 UOS 逐文件包。

v1.2.4 是首个带逐文件布局和稳定启动器的完整基线。v1.2.3 及更早来源会自动回退
完整 DEB；首次逐文件候选为 `v1.2.4 -> 后续版本`。
v1.1.0/v1.1.1 始终只能取得完整 DEB，其账号和登录会话不参与更新数据迁移。

如果以后由 UOS 主控正式发布 Windows 增量包，在 Windows 主控导出
`windows-delta-base-<版本>.zip`，带到 UOS 导入“Windows 基线包”即可；反向流程同理。
两类基线按钮在两端均可用，基线包不需要放入客户端安装目录。
...
- UOS 逐文件来源为空，或真实发布基线、逐文件包/完整回退报告全部一致；
- 当前 Windows/UOS 主控能访问更新 API，并且 API 已支持
  `X-IntDemo-Platform` 双端清单；
- 填写“仅允许更新的当前版本”时，API 还必须通过
  `/updates/capabilities.json` 声明 `source-version-targeting-v1`；
- 发布 UOS 逐文件包时，API 必须声明 `uos-file-update-v2`；
- 已填写可用的发布服务器 SSH 主机、更新目录及可选 SSH 私钥。

发布器先检查远程通道不能覆盖同版本或更高版本，再用带 SSH 保活的 SFTP 上传
缺少的产物。界面日志会显示上传字节数和百分比；断线会自动重试最多 6 次，并在
稳定的远程临时目录保留 `.part` 文件。同一版本、提交和产物重新点击发布时会从
已有字节位置继续；已完整断点必须通过 SHA-256 才会复用，错误断点只重传对应
文件。远程已有同名但不同哈希的正式文件仍会被拒绝。双端清单最后原子安装，并带
并发变更保护；远程清单完整哈希验证成功后，才把 Windows 本次快照保存为正式发布
快照。
...
“仅允许更新的当前版本”支持逗号、中文逗号、分号或空格分隔的精确 `x.y.z`
版本；留空表示不限制。填写后，只有请求中当前版本精确位于列表里的客户端会得到
更新，其他版本和未上报版本得到 HTTP 204。v1.1.0/v1.1.1 已会上报版本并正确处理
HTTP 204，无需更新客户端。这个筛选按版本而不是按设备，同一版本的所有客户端
行为一致；它只作用于更新清单，不改变账号、登录会话、权限和业务同步。
...
- UOS 缺少真实增量基线：在保存来源版本发布收据的任一主控导出“已发布基线包”，
  再到需要构建的一端导入；不要重新构建旧版本。
- 缺少增量来源快照：从保存真实发布资料的 UOS 发布机恢复该版本快照；不要重建。
- 发布 API 不支持双端平台：先部署当前服务端更新清单接口，再发布双端安装包。
- 定向发布提示服务器能力不足：先升级 API 与 Caddy，使
  `/updates/capabilities.json` 可从客户端服务地址访问；不要绕过预检。
- 提示缺少 `sftp`：执行 `sudo apt install openssh-client`，确认 `ssh`、`sftp` 都可用
  后重新打开发布器。
- 大文件上传中断：先等待发布器自动重试；如果最终失败，不要删除服务器的
  `.incoming` 目录，网络恢复后以相同版本、提交和构建结果重新点击“双端发布”即可
  续传。
