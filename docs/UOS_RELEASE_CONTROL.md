...
正式清单始终保留完整 DEB，v1.2.1 之后还可在 UOS ARM64 真机为一个精确来源版本
生成可选 `.intdelta`。
...
### 5.1 Windows 发布、UOS 真机制作增量补丁

当 Windows 是唯一能连接发布服务器的主控时，UOS 补丁按以下方式传递：

1. 已发布来源版本 `N` 后，在 Windows 发布器填写“UOS 增量来源”为 `N`，点击
   “导出已发布基线包”。得到
...
3. 在 UOS 上构建目标版本 `N+1`。发布器调用 ARM64 补丁引擎生成并立即回放补丁；
   重建结果必须与目标 DEB 逐字节一致，且补丁必须小于完整 DEB 的 50%。
4. 把 UOS 标准结果 ZIP 带回 Windows，点击“导入统信构建包”。发布器自动识别
   `N`，同步“UOS 增量来源”，并校验 DEB、补丁和回放报告。
5. Windows 取得同提交的 Windows 结果后统一发布。不要单独复制或手工选择裸
   `.intdelta`，也不要在 Windows 上制作正式 UOS 补丁。

补丁算法本身可以在其他系统处理字节，但正式发布必须验证随 UOS 客户端分发的
ARM64 引擎、真实目标 DEB、资源占用和回放结果，所以当前发布器故意只在 UOS ARM64
真机生成正式补丁。v1.2.1 是过渡完整版本；首次候选增量为
`v1.2.1 -> v1.2.2`，v1.1.0/v1.1.1 只能取得完整 DEB。
...
- UOS 增量来源为空，或真实发布基线、补丁和 ARM64 回放报告全部一致；
