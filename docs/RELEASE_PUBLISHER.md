...
6. 便携包默认不构建，偶尔需要时再勾选。Windows 增量来源与“UOS 逐文件来源”
   分别填写。v1.2.4 是首个 UOS 逐文件基线；来源为 v1.2.3 或更早版本时会自动只
   发布完整 DEB，不会再因缺少补丁中断。后续来源必须存在真实发布 DEB
...
操作栏的“导入 Windows 基线包”“导出 Windows 基线包”和 UOS 基线按钮在
Windows、UOS 两端都可用。导出必须使用真实成功发布版本的快照/安装包及发布收据；
导入会校验版本、文件大小、SHA-256 和收据，随后把基线写入当前发布器的 `dist`。
因此，未来由 UOS 负责正式发布时，不需要先把 Windows 基线导入 UOS；只需在拥有
真实来源资料的一端导出对应平台基线，再带到另一端导入即可。基线包只服务于发布器
制作增量包，客户端仍只需安装一个完整安装包，后续更新由客户端自动处理。

### 在 UOS 真机制作逐文件包、回到 Windows 发布

完整 DEB 和逐文件布局必须在 UOS ARM64 真机生成。正式逐文件包需要对真实 ARM64
业务程序、运行库和 Chromium 进行布局计算与组装回放，因此仍固定在 UOS
真机完成。Windows 只负责导入已验证结果和远程发布。

从已发布版本 `N` 制作下一版本 `N+1` 时：

1. 在 Windows 发布器填写“UOS 逐文件来源”为 `N`，点击“导出 UOS 基线包”。
...
2. 把基线 ZIP 带到 UOS ARM64 真机，点击“导入 UOS 基线包”。文件名、大小、
   SHA-256、版本和发布收据不一致时拒绝导入；同版本已有不同基线时也不会覆盖。
3. UOS 代码切到目标版本 `N+1`，保持“UOS 逐文件来源”为 `N`，构建 UOS 结果包。
   发布器会生成目标完整 DEB，调用 `build-files.sh` 逐文件比较并回放 `.intlayer`；
   只有完整目标文件树校验成功且补丁小于完整 DEB 的 50% 才写入结果包。
4. 把 `uos-build-result-*.zip` 带回 Windows，点击“导入统信构建包”。Windows
   发布器会自动识别结果中的逐文件来源和完整包回退状态，无需单独导入裸文件。
5. Windows 同时取得匹配的 Windows 构建结果后执行“双端发布”。正式清单始终
   保留完整 DEB；没有逐文件能力、版本不匹配或本机基线不可用的客户端仍走完整包。

`v1.2.4` 是首个逐文件基线完整版本。v1.2.3 及更早版本没有
`file-layout.json`，升级 v1.2.4 时都按原流程取得完整 DEB；首个逐文件候选从
v1.2.4 → 后续版本开始。
...
v1.2.4 实施 UOS ARM64 应用内逐文件协议、构建和发布能力，并作为首个完整逐文件
基线。首个新格式 `.intlayer` 从 v1.2.4 → 后续版本开始。三层协议和旧整 DEB
差分协议仅保留兼容。实施
方案与验收规则见
...
- `build-files.sh` 已成功组装回放，完整目标文件树与逐文件布局一致；
- 稳定启动器、在线配置与 CA 指纹未改变；发生变化时自动发布完整 DEB；
- `.intlayer` 小于目标完整 DEB 的 50%；未达门槛时自动发布完整 DEB；
- 客户端声明 `uos-file-update-v2` 能力且当前版本精确匹配来源版本。

跨机器制作时，真实来源 DEB 与发布收据必须通过“已发布基线包”从 Windows 发布
主控带到 UOS；目标 DEB、逐文件包生成和回放验证全部在 UOS ARM64 真机完成，再把
标准 UOS 结果 ZIP 带回 Windows。不能只复制裸补丁，也不能在 Windows 重建旧 DEB 代替
已发布基线。

UOS 客户端不直接修改安装目录。它在用户数据目录按文件清单组装完整程序版本并经
稳定启动器试运行；失败时回滚，基线、空间或校验异常时自动下载完整 DEB。没有能力请求头的
