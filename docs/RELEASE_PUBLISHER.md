...
6. 便携包默认不构建，偶尔需要时再勾选。Windows 与 UOS 增量来源分别填写；
   v1.2.1 是 UOS 过渡完整版本，应将 UOS 增量来源留空。后续 UOS 来源必须存在
   真实发布 DEB 和发布收据。导入 Windows 构建包时会自动识别是否含便携包。
7. 在“输出类型”中选择：
...
### 在 UOS 真机制作补丁、回到 Windows 发布

完整 DEB 本来就必须在 UOS ARM64 真机生成。二进制差异本身不依赖桌面系统，
但正式补丁还必须使用客户端内置的 ARM64 补丁引擎立即回放，并核对重建 DEB 的
大小、SHA-256、耗时和体积门槛；因此发布器把正式补丁的生成和回放都固定在
UOS ARM64 真机。Windows 只负责导入已验证结果和远程发布。

从已发布版本 `N` 制作下一版本 `N+1` 时：

1. 在 Windows 发布器填写“UOS 增量来源”为 `N`，点击“导出已发布基线包”。
...
3. UOS 代码切到目标版本 `N+1`，保持“UOS 增量来源”为 `N`，构建 UOS 结果包。
   发布器会生成目标完整 DEB，调用 `build-delta.sh` 生成并回放 `.intdelta`；只有
   逐字节重建成功且补丁小于完整 DEB 的 50% 才把补丁写入结果包。
4. 把 `uos-build-result-*.zip` 带回 Windows，点击“导入统信构建包”。Windows
   发布器会自动识别结果中的增量来源并同步界面，无需单独导入裸补丁。
5. Windows 同时取得匹配的 Windows 构建结果后执行“双端发布”。正式清单始终
   保留完整 DEB；没有增量能力、版本不匹配或本机基线不可用的客户端仍走完整包。

`v1.2.1` 是过渡完整版本，不能从 v1.1.0/v1.1.1 制作面向旧客户端的应用内 UOS
补丁；这两个旧版没有增量能力，会按原流程取得完整 DEB。首个候选增量应在
v1.2.1 真机安装并建立基线后，验证 `v1.2.1 -> v1.2.2`。
...
v1.2.1 已实现 UOS ARM64 应用内增量协议、构建和发布能力，但 v1.2.1 是过渡完整
版本，正式清单仍不发布 UOS 补丁。实施方案与实测结果见
...
- `build-delta.sh` 已成功回放，目标 DEB 大小和 SHA-256 逐字节一致；
- `.intdelta` 小于目标完整 DEB 的 50%；未达门槛时只允许发布完整 DEB；
- 客户端声明 `uos-deb-xdelta-v1` 能力且当前版本精确匹配来源版本。

跨机器制作时，真实来源 DEB 与发布收据必须通过“已发布基线包”从 Windows 发布
主控带到 UOS；目标 DEB、补丁生成和回放验证全部在 UOS ARM64 真机完成，再把标准
UOS 结果 ZIP 带回 Windows。不能只复制裸补丁，也不能在 Windows 重建旧 DEB 代替
已发布基线。

UOS 客户端不直接修改安装目录。它在用户缓存中重建完整 DEB，校验后仍交给系统
安装器；基线、补丁、空间或引擎异常时自动下载完整 DEB。没有能力请求头的
v1.1.0、v1.1.1、v1.2.0 客户端始终获得完整包。
