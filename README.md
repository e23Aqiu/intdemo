...
当前版本：`1.2.2`。验证码训练支持标准模式（OpenCV HOG + 线性 SVM）和可选强化模式
...
- “系统设置”集中展示账号资料、版本信息、更新内容、检查按钮和下载进度；
  ...
  或 `.deb`。v1.2.2 起 UOS 客户端可声明 `uos-layered-v1` 能力：本地布局与真实
  发布基线精确匹配时接受 `.intlayer`，在用户目录组装业务程序、运行库和 Chromium
  三层并经稳定启动器试运行；其他情况自动使用完整 DEB。v1.1.0、v1.1.1、v1.2.0
  和 v1.2.1 的更新行为保持不变。
...
`.intlayer` 时使用下文的“双端打包发布器”，由标准 UOS 结果 ZIP 携带分层报告和
可选分层包。

发布脚本在同一个 `test.json` 的 `platforms` 中原子写入 Windows x64 和 UOS
ARM64 完整包；同时把 Windows 完整包保留在顶层以兼容旧客户端。Windows 可选差异
包继续写入 `deltas`；UOS v1.2.2 起由打包发布器生成可选 `layered_updates`，客户端
必须声明 `uos-layered-v1`，且分层包必须低于完整 DEB 的 50%。来源无布局、引导文件
变化或其他条件不满足时，发布器、服务器和客户端都会选择完整包。
...
`from_version`。v1.2.2 UOS 客户端还会发送包含 `uos-layered-v1` 的
