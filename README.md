...
当前版本：`0.2.6` 在线同步测试版。`v0.1` 的本地数据库和发布包保持不变，
...
另有一条与业务同步隔离的授权验证码学习通道：默认关闭；只有管理员开启后，
客户端才会上传已确认成功的验证码图片及数字答案或文字点选坐标。失败尝试只发送
不含图片和答案的模型效果计数，用于计算线上成功率。详见
...
- 登录页的“离线登录（游客）”无需账号或密码，只开放一键业务处理；游客
...
- 登录页支持记住密码和自动登录；等待登录、改密及在线管理请求时会显示
  模态加载状态，避免误以为程序无响应。
- 无账号密码时可选择“离线登录（游客）”，只处理并保存用户主动选择的业务
...
- 管理员侧边栏提供“机器学习”页面：可随时开关客户端样本上传，实时查看样本
...
的 API、同步及 WebSocket 连接以验证离线行为；构建与发布权限不会进入业务客户端。
