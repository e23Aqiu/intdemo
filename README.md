...
- “系统设置”集中展示账号资料、版本信息、更新内容、检查按钮和下载进度；
  ...
  或 `.deb`。v1.2.1 起 UOS 客户端可声明应用内增量能力：只有本地存在正式哈希匹配
  的基线 DEB 时才接受 `.intdelta`，重建并校验完整 DEB 后仍调用系统安装器；其他
  情况自动使用完整 DEB。v1.1.0、v1.1.1 和 v1.2.0 的更新行为保持不变。
...
ARM64 完整包；同时把 Windows 完整包保留在顶层以兼容旧客户端，并把可选差异包
写入各自平台的 `deltas`。Windows 沿用版本精确匹配规则；UOS 还必须声明
`uos-deb-xdelta-v1` 能力，且补丁必须低于完整 DEB 的 50%。不满足条件时服务器和
客户端都会选择完整包。
...
`from_version`。v1.2.1 UOS 客户端还会发送 `X-IntDemo-Update-Capabilities`；旧 UOS
客户端没有该请求头，因此即使版本号命中也只会获得完整 DEB。
