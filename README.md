...
- 运行期间定期保存计时心跳；程序异常退出后会保留最后一次心跳前的累计时间，并在下次启动时将旧运行标记为异常中断。
- 数据仪表盘只统计三个步骤全部成功的批次，按站点和完成日期范围以一位小数小时展示总用时、有效用时、每条平均用时，以及相对人工 260 条耗时 6 小时基准的效率提升比例；批次完成后会纳入此前停止、异常或重试产生的用时，悬停卡片可查看精确到毫秒的明细，升级前无计时记录的历史条数不参与平均值计算。
- 将运行参数集中在独立标签页，并可实际启动内置 Chromium 执行健康检查。
- 实时展示数据预览、总体进度、分步骤状态和运行日志。
- 数据预览和流水线日志可分别收起或展开，按当前工作重点调整页面空间。
- 流水线日志跨多次执行保留最近 5000 行，并用执行次数分隔；浏览器被手动关闭、目标页返回 HTTP 404 或导航超时时，会自动重启内置 Chromium 并重试当前记录，恢复失败时停止当前步骤而不会静默跳过。
- 第二步营运查询的页面等待上限为 60 秒；验证码题目和图片完全加载并保持稳定后才开始自动或人工处理，验证码通过后也会继续等待企业结果真正返回；纯技术超时会重试或记录失败，不会误弹公司人工录入框。
