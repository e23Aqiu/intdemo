...
当前版本：`1.2.3`。验证码训练支持标准模式（OpenCV HOG + 线性 SVM）和可选强化模式
...
Windows 负责正式发布、UOS 无法直连发布服务器时，可在 Windows 点击“导出已发布
基线包”，把真实旧 DEB 与发布收据带到 UOS ARM64 真机；UOS 导入后生成目标 DEB、
制作并回放验证 `.intdelta`，再把标准 UOS 结果 ZIP 带回 Windows 导入发布。正式
UOS 补丁不在 Windows 生成；v1.2.1 是过渡完整版本，首个候选增量为
v1.2.1→v1.2.2。
