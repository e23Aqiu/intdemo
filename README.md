...
当前版本：`1.1.0`。验证码训练支持标准模式（OpenCV HOG + 线性 SVM）和可选强化模式
（Tiny CNN + ONNX）。强化组件由管理员按目标平台本地安装；发布器可选择
`trainer-trust.json` 并将其随 Windows/UOS 安装包写入，组件本身不上传服务器。
机器学习页面可查看当前模式、算法与组件状态，并提供安装、自检和卸载入口；卸载
组件不会删除样本或已训练模型。强化组件的详细构建和签名规则见
