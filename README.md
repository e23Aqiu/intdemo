...
当前版本：`1.2.1`。验证码训练支持标准模式（OpenCV HOG + 线性 SVM）和可选强化模式
...
- 仪表盘继续从本地 SQLite 缓存按站点和日期范围查询、展示及导出汇总数据，断网时仍可查看最近一次同步结果；仪表盘和数据中心默认只看黄牌车辆，可取消勾选切换为全部车辆，并按程序账号记住上次选择；升级前的历史数据按黄牌处理。数据中心复用未变化的统计查询，后台同步仅在确有新数据时刷新；刷新后保留图表/完整数据页签、表格列宽、滚动位置和当前选中项。
