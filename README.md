...
当前版本：`1.0.7`。`v0.1` 的本地数据库和发布包保持不变，
...
- 数据预览区提供“回填业务数据”：仅当当前 Excel 的“车辆所有人/企业”列
  全部非空时启用。确认腾讯文档链接后，程序先检查按当前程序账号保存的腾讯文档
  登录状态；未登录时打开浏览器供用户登录，检测到登录完成后会自动重新执行。
  已登录时在后台核对腾讯文档末尾待回填行数与 Excel 数据行数，完全一致后，仅在
  腾讯文档实际存在的公司名称、法人/负责人、地址、电话列中回填对应内容，缺少地址等
  列时直接跳过且不会新增在线列，其他内容保持不变，并显示进度及回读校验结果。
  腾讯文档导入、登录状态复用和业务数据回填同样可在程序的离线登录模式下使用。
