...
当前版本：`1.3.0`。验证码训练支持标准模式（OpenCV HOG + 线性 SVM）和可选强化模式
（Tiny CNN + ONNX）。强化组件由管理员按目标平台通过单个 `.inttrainer` 文件本地安装，
不需要配套 JSON，组件本身不上传服务器。
机器学习页面可查看当前模式、算法与组件状态，并提供安装、自检和卸载入口；卸载
组件不会删除样本或已训练模型。强化组件的详细构建和校验规则见
[强化训练组件](docs/ENHANCED_TRAINER.md)。
`v0.1` 的本地数据库和发布包保持不变，
...
自动模型成功率，人工操作不上传、不统计；第三档才会在成功后上传验证码图片及
数字答案或文字点选坐标。详见
...
- “系统设置”集中展示账号资料、版本信息、更新内容、检查按钮和下载进度；
  ...
  或 `.deb`。从 v1.2.4 起，UOS 客户端可声明 `uos-file-update-v2` 能力：
  本地布局与真实发布基线精确匹配时接受只包含新增/变化文件和删除清单的
  `.intlayer`，在用户目录组装完整版本并经稳定启动器试运行；其他情况自动使用完整
  DEB。v1.1.0、v1.1.1 及其他无新能力客户端的更新行为保持不变。
...
- 在线账号按“管理员 → 路段管理员 → 中心站”分级。管理员可管理全部账号和路段，
  并为路段管理员、中心站设置“本人 / 本路段 / 全部”数据范围；路段管理员不显示
  机器学习界面，只能管理自己路段的中心站，且只能授予“本人 / 本路段”范围。
  路段管理员的账号显示名就是路段分类名，只有存在对应路段管理员的分类才能分配给
  中心站；中心站也可保持“未分配”并由管理员直接管理。永久删除最后一个路段管理员
  后，该路段全部中心站会变为“未分配”，新建同名路段管理员后仍需手动重新分配；
  仅归档管理员账号不会解除关系。账号仍可设置登录设备上限（默认 10000 台），并
  支持密码重置、启停、归档/恢复、统计重置、异常设备撤销和审计。账号列表可按
  在用/归档状态、角色和所属路段组合筛选；“未分配”包含直属管理员的中心站和
  测试账号，全局管理员的所属路段显示为“不适用”。测试账号始终为
  “未分配”，路段管理员不能查看或管理测试账号。路段管理员账号自身不参与任何
  业务统计；其历史缓存和新产生数据都不会进入条数、占比、效率、违规原因或站点
  分布，其显示名和登录名也不会出现在仪表盘或数据中心的站点下拉、表格、图表及
  Excel 导出中；但拥有“本路段 / 全部”数据范围时仍可查看范围内中心站的统计。
- 仪表盘继续从本地 SQLite 缓存按站点和日期范围查询、展示及导出汇总数据，断网时
  仍可查看最近一次同步结果；“各站查询统计”可独立切换今日/累计口径，同时更新
  该面板的数据表和占比图，不改变其他全局指标。仪表盘和数据中心默认只看黄牌车辆，
  可取消勾选切换为全部车辆，并按程序账号记住上次选择；升级前的历史数据按黄牌处理。
  数据中心复用未变化的统计查询，后台同步仅在确有新数据时刷新；刷新后保留图表/
  完整数据页签、表格列宽、滚动位置和当前选中项。
- 对同一份 `.xlsx` 文件依次执行运输证查询、营运企业回填和爱企查信息补齐。
- 业务表格路径框支持直接拖入单个 `.xlsx` 文件并立即加载数据预览。
...
- 三步结果均使用 `openpyxl` 定向回写目标单元格，并通过同目录临时文件原子替换保存，避免中途停止时重建整表或丢失下方数据；新版“备注”列（同时兼容旧版“已协助补缴”列）后的带格式空列会优先复用，旧版追加到右侧的结果列会在再次写入时自动迁回。
- 重新执行流程时，已有运输证号、已有车辆所有人/企业信息，或“备注”/“已协助补缴”任一列已有内容的记录会跳过运输证查询，避免重复访问网站。
...
- “运行模式”下的“爱企查兼容模式”默认开启：步骤 3 在 Windows 优先使用
  Microsoft Edge、找不到 Edge 时使用 360 浏览器，在统信 UOS 使用 Deepin
  浏览器；关闭该选项后，步骤 3 改用项目内置 Chromium。统信还会识别系统桌面上的
  `org.deepin.browser.desktop` 启动项并读取其 `Exec`/`TryExec`，未找到对应本机浏览器时，
  程序会提示关闭兼容模式后重试。该设置按程序登录账号分别保存。
...
- 管理员侧边栏提供“机器学习”页面：可随时切换三档客户端上报策略，实时查看样本
  ...
  ZIP 数据集，并分别训练、应用或删除数字与文字点选候选模型。训练终端实时显示
  样本下载进度、样本过滤、强化模式逐轮损失/准确率/耗时，以及留出集逐条识别结果
  与真实结果；强化训练轮次可在 1～200 之间设置并按本机持久保存。
...
构建产物会自动携带该浏览器，供步骤 1、步骤 2 以及关闭爱企查兼容模式后的
步骤 3 使用。默认开启的爱企查兼容模式会使用 UOS 自带的 Deepin 浏览器；若未找到，
可在运行设置中关闭该模式并使用内置 Chromium。
...
最后三条命令会将步骤 1、步骤 2 及爱企查兼容模式关闭时使用的 Chromium 安装到
项目虚拟环境中。爱企查兼容模式默认开启，Windows 会优先调用本机 Edge，其次调用
360 浏览器；如果都未找到，可关闭该模式并使用上述内置 Chromium。
...
服务器还会幂等创建并启用以下中心站账号，统一归属“广深高速”；所有初始账号
首次登录均必须修改密码：
...
上面的 `publish-update.ps1` 兼容命令用于直接发布 UOS 完整 DEB；需要跨机制作和发布
`.intlayer` 时使用下文的“双端打包发布器”，由标准 UOS 结果 ZIP 携带逐文件报告和
可选逐文件包。

发布脚本在同一个 `test.json` 的 `platforms` 中原子写入 Windows x64 和 UOS
ARM64 完整包；同时把 Windows 完整包保留在顶层以兼容旧客户端。Windows 可选差异
包继续写入 `deltas`；UOS v1.2.4 完整 DEB 写入逐文件基线，后续由打包
发布器生成可选 `file_updates`。客户端必须声明 `uos-file-update-v2`，且逐文件包必须
低于完整 DEB 的 50%。来源无新布局、引导文件变化或其他条件不满足时，发布器、
服务器和客户端都会选择完整包。旧 `uos-layered-v1` 仅保留清单兼容。
...
下载并启动新安装包进行升级。下载可随时停止并显示实时速度；普通更新点击“立即更新”后
仍可切换到“后台更新”，下载期间业务页保持可用。强制更新从下载到校验完成会锁定业务页，
但仍可选择退出登录或退出应用；只有开始安装时才要求保存并停止当前业务。
...
`from_version`。新版 UOS 客户端还会发送包含 `uos-file-update-v2` 的
`X-IntDemo-Update-Capabilities`；旧 UOS 客户端没有该能力，因此即使版本号命中也
只会获得完整 DEB。用户首次仍只需安装一个 DEB，后续补丁由客户端自动处理，不需要
安装外部插件。

发布器的“仅允许更新的当前版本”可填写精确版本列表，例如只允许 v1.1.0 获取
新版本而让 v1.1.1 继续得到“暂无更新”；留空时保持原来的全部版本可见。使用该
功能前必须先升级 API 与 Caddy，发布器会检查
`/updates/capabilities.json`，不支持时拒绝发布。v1.1.0/v1.1.1 已会上报当前版本
并能处理 HTTP 204，不需要升级客户端，也不会丢失已有登录状态。
...
任一端需要为另一端制作增量包时，都可以点击对应的“导出基线包”，把真实发布快照
（或 UOS DEB）和发布收据带到另一端导入。UOS 逐文件补丁仍必须在 UOS ARM64 真机
生成和回放验证，Windows 只负责导入结果并发布；未来由 UOS 正式发布 Windows 增量
包时，流程完全对称。基线包只由发布器使用，不放入客户端目录；客户端仍只需安装
一个完整安装包，后续补丁由客户端自动处理。
