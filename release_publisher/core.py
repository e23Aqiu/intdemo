...
class ReleaseOptions:
    ...
    @property
    def uos_layer_installer(self) -> Path | None:
        if not self.uos_delta_from_version:
            return None
        return (
            self.repo_root
            ...
            / "uos-arm64"
            / (
                f"IntDemo-UOS-arm64-Layers-{self.uos_delta_from_version}"
                f"-to-{self.version}.intlayer"
            )
        )

    @property
    def uos_layer_report(self) -> Path | None:
        package = self.uos_layer_installer
                    ...
                    uos_steps.append(
                        CommandStep(
                            ...
                            arguments=(
                                ...
                                str(
                                    options.repo_root
                                    / "dist/uos-arm64/package"
                                    / f"IntDemo-UOS-arm64-{options.version}"
                                ),
                                "--from-version",
                                options.uos_delta_from_version,
                                "--target-version",
